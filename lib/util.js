...
function iconvExec(command, option, callback) {
  if (option.iconvEncoding) {
    const iconvEncoding = option['iconvEncoding'];
    option['encoding'] = "buffer";
    exec(command, option, (err, stdout) => {
      let data = stdout;
      data = iconv.decode(stdout, iconvEncoding);
      callback(err, data)
    });
  } else {
...
function iconvString(data, encoding) {
...
exports.iconvExec = iconvExec;
exports.iconvString = iconvString;
